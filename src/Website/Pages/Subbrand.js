      ...
      <div className='container-fluid mt-5'>
        <div className='row'>
          <div className='shop-bg'>
            <h1 className='package1'>{brands.brand_name}</h1>
            <Link className='pa'><span style={{ color: "white" }}> <i class="fa-solid fa-house fa-beat me-2" ></i></span>Home /<span>{brands.brand_name}</span></Link>
                    ...
                    {Cat.slice(0, 18).map((cat, index) => (
                      ...
                      <li key={index} className={activeIndex === index ? 'open' : ''} style={{ textDecorationLine: "none" }}   >
                        <Link onClick={() => handleItemClick(index)} to='#' style={{ textDecoration: 'none', color: "black" }}>
                          ...
                          {/* {Count((count) => count.product_category_id == cat.category_id).map((count) => (
                            (count.cat_count)
                         ))}*/}
                        </Link>
                        ...
                        {/* <ul type="none" style={{ display: activeIndex === index ? 'block' : 'none' }}>
                          submenu {
                            Sub((sub) => sub.subcategory_category_id === cat.category_id).map((sub) => (
